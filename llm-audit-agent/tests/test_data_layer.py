...
def test_run_writes_real_results_to_a_separate_file(tmp_path):
    ...
    (ROOT / "results" / "latest-real.json").unlink(missing_ok=True)


def test_authored_run_still_writes_latest_json():
    from src import demo
    r = demo.run()
